...
- Create Kubernetes cluster in two types: zonal or regional 
- Create a user defined Kubernetes node groups
...
Kubernetes module requires a following input variables:
 - VPC network id
 - VPC network subnets ids
 - Master locations - a list of maps with zone name and subnet id for each location.
 - Node groups - a list of node group maps with any number of parameters

```
Master locations must have only ONE or THREE locations! One for Zonal and three for Regional cluster!
```

<b>Notes:</b>
  - If node groups version is missing, `cluster version` will be used instead of.
  - All node groups are able to define own locations. These locations will be used instead of master locations.
  - If own location wasn't define for node group with `auto scale` policy, location for this group will be automatically generated from master locations list.
  - If node groups list have more than three groups, locations for them will be assigned from the beggining of the master locations list. So, all node groups will be distributed in a range of master locations list.
  - All three master locations will be used for `fixed scale` node groups.

## Node Group definition

`node_groups` is a list of maps for each node group. You are able to determine any parameter for each node group, but all of them have default values. So, an empty node group object will be created using default values.
For example, `example 2` defines 7 node groups with own parameters. You might create any number of node groups, which are limited by the capabilities of Yandex Kubernetes service. If `node_location` parameter is absent, location will be automatically assigned from master locations list. 
